...
// Parser for TVC Pro Driver referrals. Works on both pasted email text and the
// text layer extracted from a PDF. PDFs often glue labels to values
// ("Address3650 WASHINGTON ST") and list the firm's Attorney Info before the
// member's, so we tokenize the document by known labels (whitespace-tolerant)
// and then select the correct occurrence of each field by its position.

interface LabelDef {
  key: string;
  re: string; // regex source; spaces matched as \s*
}

// Ordered longest / most-specific first so e.g. "Driver's License State" wins
// over "Driver's License", and "Member Info" over "Member".
...
function stripNoise(raw: string): string {
  let t = raw
    .replace(/https?:\/\/[^\s…]+/gi, ' ') // urls (stop at the page ellipsis)
    .replace(/This message's attachments[\s\S]*?by phone\.?/i, ' ')
    .replace(/\d{1,2}\/\d{1,2}\/\d{2,4},\s*\d{1,2}:\d{2}\s*[AP]M/gi, ' ') // gmail header time
    .replace(/ironrocklaw\.com Mail/gi, ' ')
    .replace(/--\s*\d+\s*of\s*\d+\s*--/gi, ' ')
    .replace(/…?\s*\d\/\d\b/g, ' '); // page "1/5" markers
  // Everything from the activity log onward is noise (and duplicated threads).
...
function tokenize(text: string): Token[] {
  ...
  for (let i = 0; i < matches.length; i++) {
    const m = matches[i];
    const matchText = m[0];
    const key = canonical(matchText);
    const start = (m.index ?? 0) + matchText.length;
    const end = i + 1 < matches.length ? (matches[i + 1].index ?? text.length) : text.length;
    tokens.push({ key, value: text.slice(start, end).trim() });
  }
  return tokens;
}
...
export function parseTvc(raw: string): ParseResult {
  ...
  // Case number (from anywhere in the text).
  const caseM = text.match(/TVC\s*Legal\s*Case:?\s*(\d+)/i);
  if (caseM) f.tvcCaseNumber = caseM[1];

  // Member identity is anchored on Birthdate: the Address and Member name
  // immediately preceding it belong to the member (not the firm).
  ...
  // Member mobile: the Mobile value that isn't the attorney's "DO NOT GIVE" line.
  ...
  // Court block — use anchored regexes on the cleaned text. The word "County"
  // can appear inside a court name ("Pulaski County DC"), so token splitting is
  // unreliable here; the Court Name / Phone / City / State labels are not.
  ...
  // Court date — parse the Court Dates table value.
  const cd = first('courtDates');
  if (cd) {
    const dm = cd.match(/(\d{1,2}\/\d{1,2}\/\d{2,4})\s*(\d{1,2}:\d{2}\s*[AP]M)?\s*([A-Za-z][A-Za-z\- ]*)?/);
    if (dm) {
  ...
  // Tickets / charge live in the Case Coverage Type value (after the "25%").
